...
## refactor
Description: 自动化重构工具，支持多种代码重构操作。该工具可以帮助改善代码结构、提高可读性和可维护性，同时保持功能不变。
...
- operation: (required) 要执行的重构操作类型（rename, extract_method, extract_variable, inline_variable, move_code, extract_interface, add_parameter, remove_parameter, change_signature, split_class, merge_classes）
- target: (required) 重构的目标（如要重命名的标识符名称、要提取的代码块等）
- new_name: (optional) 新名称（用于rename操作）
- start_line: (optional) 起始行号（从1开始，用于extract_method、move_code等操作）
- end_line: (optional) 结束行号（从1开始，用于extract_method、move_code等操作）
- destination_file: (optional) 目标文件路径（用于move_code、extract_interface等操作）
- destination_line: (optional) 目标位置行号（用于move_code操作）
- method_name: (optional) 提取的方法名称（用于extract_method操作）
- variable_name: (optional) 变量名称（用于extract_variable、inline_variable操作）
- interface_name: (optional) 接口名称（用于extract_interface操作）
- preview_only: (optional) 是否仅预览重构结果而不实际执行（默认false）
- preserve_comments: (optional) 是否保留注释（默认true）
- update_references: (optional) 是否自动更新所有引用（默认true）
...
## Example 7: Refactoring code (renaming a variable)
...
## Example 8: Extracting a method from code

<refactor>
<file_path>src/components/UserProfile.tsx</file_path>
<operation>extract_method</operation>
<target>validateUserData</target>
<method_name>validateUserData</method_name>
<start_line>45</start_line>
<end_line>55</end_line>
</refactor>

## Example 9: Performing code review
